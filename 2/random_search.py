...
from cnn_mnist import mnist, test, train_and_validate
...
class MyWorker(Worker):
    ...
    def get_configspace():
        ...
        num_filters = CSH.UniformIntegerHyperparameter(
            "num_filters", lower=2 ** 3, upper=2 ** 6, log=True
        )
...
parser.add_argument(
    "--budget",
    type=float,
    help="Maximum budget used during the optimization, i.e the number of epochs.",
    default=6,
)
parser.add_argument(
    "--n_iterations",
...
rs = RandomSearch(
    ...
    min_budget=int(args.budget),
    max_budget=int(args.budget),
)
