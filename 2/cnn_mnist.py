...
def unhot(one_hot_labels):
    np.argmax(one_hot_labels, axis=1)
...
def train_and_validate(
    ...
    model = tf.keras.models.Sequential(
        [
            tf.keras.layers.Conv2D(
                num_filters, filter_size, 1, padding="same", activation=tf.nn.relu
            ),
    ...
    model.fit(
        ...
        epochs=num_epochs,
        callbacks=[
            tf.keras.callbacks.TensorBoard(log_dir="./tensorboard.log/%s" % run_id),
            tf.keras.callbacks.LambdaCallback(
                on_epoch_end=lambda epoch, logs: learning_curve.append(
                    {
                        "epoch": epoch,
